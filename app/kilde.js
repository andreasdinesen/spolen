/*
 * spolen henter sin egen kode.
 *
 * Indtil v22 fulgtes app-koden og runen ad: en ny udgave betoed en ny rune,
 * som Andreas skulle opdatere i panelet. Runen BAR ikke koden - den hentede
 * den fra en tag - saa trinnet var ren ceremoni: et nyt versionsnummer i en
 * YAML, hvis eneste opgave var at pege paa en anden tag.
 *
 * Nu goer serveren det selv. Runens `startup` koerer det her modul FOER
 * serveren, saa **en genstart er opdateringen**. Runen skal kun udgives,
 * naar selve runen aendrer sig.
 *
 * Designet er dodas (v82/v83), overtaget ordret paa de punkter, der er
 * MAALT der - ikke gaettet her.
 *
 * -- De tre regler ------------------------------------------------------
 *
 * 1. **En fejl maa aldrig kunne forhindre serveren i at starte.** Alt
 *    herinde ender med exit 0. Kan GitHub ikke naas, koerer den kode, der
 *    ligger. En netvaerksfejl paa serveren maa udsaette en opdatering -
 *    aldrig slukke for appen.
 * 2. **Der byttes ALDRIG halvt.** Der pakkes ud i en frisk mappe ved siden
 *    af app/, den tjekkes, og foerst derefter skiftes navnene. Mellem de to
 *    omdoebninger ligger app/ under `.spolen-gammel` - og runens startup
 *    saetter den tilbage, hvis vi doer praecis der.
 ...
 *    tilbage fra en daarlig udgivelse: skriv tallet i panelet, genstart.
 *
 ...
 * tilfaeldigvis naevner foerst (det er v9, og det er alfabetisk »nyere« end
 * v80).
 */
...
/* Foerste udgave, der indeholder DEN HER fil. Laaser man laengere tilbage,
 * forsvinder kilde.js sammen med resten af app/, og saa opdaterer en
 * genstart ikke mere. Det skal staa i loggen, FOER det sker. */
const FOERSTE_MED_KILDE = 23;

const APP = __dirname;                        // <rod>/app
const ROD = path.dirname(APP);                // runens arbejdsmappe
const MAERKE = path.join(APP, '.kode-version');
const NY = path.join(ROD, '.spolen-ny');
const GAMMEL = path.join(ROD, '.spolen-gammel');
...
/* Arkivet er ~350 kB i dag. Loftet er en spaerre, ikke en forventning. */
 ...
 *
 * Tomt, »seneste« og »latest« betyder alle det samme. Tom er STANDARDEN:
 * et felt, der SKAL udfyldes for at opfoere sig normalt, laeser man som en
 * indstilling, nogen har taget (Andreas efter doda v82).
 *
 * Alt andet end et rent tal afvises HOEJLYDT frem for at blive tolket:
 * skriver man »v21« eller »21.1«, skal man vide, at spolen ikke gjorde,
 * hvad der stod.
 */
 ...
 * install-script (eller en spolen fra foer denne aendring) - og saa staar
 * tallet i index.html, hvor build'et stempler det. Uden det fallback ville
 * hver eneste eksisterende server hente koden igen ved foerste genstart,
 * ogsaa naar den allerede var den rigtige.
 */
...
function hentSvar(url, forsoeg, ved) {
  return new Promise((ok, nej) => {
    const req = https.get(url, {
      headers: { 'user-agent': 'spolen-opdatering', accept: '*/*' },
      timeout: TIMEOUT_MS,
    }, (res) => {
 ...
 *
 * Der bladres, indtil en side ikke er fuld. GitHub sorterer tags
 * ALFABETISK - v9 staar efter v80 - saa der SKAL regnes paa hele listen
 * frem for at tage den foerste. Tog vi `liste[0]`, ville hver server rulle
 * tilbage til v9 ved naeste genstart.
 */
...
/**
 * Henter og pakker `vN` ud i .spolen-ny/ og returnerer stien til app-mappen.
 *
 * Der pakkes ud ved siden af app/ - ikke i /tmp. `mv` mellem to filsystemer
 * er en kopi, og en kopi kan afbrydes paa midten; et `rename` inden for
 * samme filsystem kan ikke. Det er hele grunden til, at temp-mappen ligger
 * her og ikke i /tmp, hvor install-scriptet har sin.
 *
...
/**
 * Er det her en hel spolen - og er det den, vi bad om?
 *
 ...
 *
 * Listen er IKKE dodas. Den skal spejle det, DENNE app faktisk indlaeser:
 * spolens server.js require'r tretten egne moduler paa modulniveau, og et
 * traee uden ét af dem ville blive godkendt, byttet ind - og saa ville
 * containeren starte og straks doe med MODULE_NOT_FOUND ved hver eneste
 * genstart (tovo, 2026-09-03). En liste, der er rigtig i forlaegget, er
 * ikke rigtig hos modtageren.
 *
 * `tests/kilde.test.js` laeser server.js' egne require-linjer og kraever, at
 * de alle staar her - saa kan listen ikke drive fra virkeligheden, naeste
 * gang nogen tilfoejer et modul.
 */
 ...
 *
 * Doer processen mellem dem, ligger den gamle app under `.spolen-gammel`,
 * og runens startup saetter den tilbage. Uden det trin ville et daarligt
 * sekund kunne efterlade en container uden app/ - og uden app/ er der
...
async function opdater(env = process.env) {
  ...
  if (har && har.version === maal) {
     ...
     * at versionen er det, og saa skal maerket kunne vise det rigtige. */
    try {
  ...
  if (maal < FOERSTE_MED_KILDE) {
    advar(`v${maal} er fra foer spolen kunne hente sin egen kode.`);
    advar('En genstart opdaterer ikke derfra - brug »Opdater spolen« i panelet for at komme videre.');
  }
...
module.exports = { oensket, installeret, nyesteTag, tjekTrae, opdater, MAERKE, FOERSTE_MED_KILDE };
